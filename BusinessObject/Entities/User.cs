...
public class User
{
    [Key] [Column("user_id")] public string UserId { get; set; } = Guid.NewGuid().ToString();
    ...
    [Required] [Column("role")] public UserRole Role { get; set; } = UserRole.Driver;

    [Required] [Column("status")] public UserStatus Status { get; set; } = UserStatus.Active;

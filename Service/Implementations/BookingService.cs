...
public class BookingService : IBookingService
{
    ...
    private readonly TimeSpan StartTime = new(8, 0, 0); // 8:00 AM
    private readonly TimeSpan EndTime = new(20, 0, 0); // 8:00 PM
    ...
    public async Task<BookingResponseDto?> CreateBookingAsync(string userId, CreateBookingDto request)
    {
        ...
        if (user == null || user.Role != UserRole.Driver ||
            user.Status != UserStatus.Active) // 1 = Customer, 1 = Active
            return null;
    ...
    // ✅ SỬA: Đổi parameter type
    public async Task<BookingResponseDto?> UpdateBookingStatusAsync(string bookingId, string staffId,
        UpdateBookingStatusDto request)
    {
        ...
        else if (request.Status == BBRStatus.Completed) booking.CompleteAt = DateTime.UtcNow;
    ...
    public async Task<List<BookingResponseDto>> GetBookingsByStatusAsync(BBRStatus status,
        BookingFilterDto? filter = null)
    {
        var query = _context.Bookings
            .Include(b => b.User)
    ...
    public async Task<BookingStatisticsDto> GetBookingStatisticsAsync(string? stationId = null,
    ...
    {
        if (!TimeSpan.TryParse(timeSlot, out var timeSlotSpan))
            return false;
